...
struct A_exp_{
	...
	union {
		...
		struct {
			A_oper oper; A_exp left; A_exp right;
		} op;
		struct {
			S_symbol typ; A_efieldList fields;
		} record;
		A_expList seq;
		struct {
			A_var var; A_exp exp;
		...
		struct {
			S_symbol var; A_exp lo,hi,body; bool escape;
		} forr;
		/* breakk; - need only the pos */
		struct {
			A_decList decs; A_exp body;
		} declare_expression;
		struct {
...
struct A_dec_ {
	...
	union {
		...
		struct {
			S_symbol var; S_symbol typ; A_exp init; bool escape;
		} var;
		A_nametyList type;
	} u;
};
